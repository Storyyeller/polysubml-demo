...
export class State {
    ...
    process(source) {
        ...
        const ret = wasm.state_process(this.__wbg_ptr, ptr0, len0);
        return ret !== 0;
    }
    /**
     ...
     */
    get_output() {
        const ret = wasm.state_get_output(this.__wbg_ptr);
        let v1;
        if (ret[0] !== 0) {
     ...
     */
    get_err() {
        const ret = wasm.state_get_err(this.__wbg_ptr);
        let v1;
        if (ret[0] !== 0) {
            v1 = getStringFromWasm0(ret[0], ret[1]).slice();
            wasm.__wbindgen_free(ret[0], ret[1] * 1, 1);
...
async function __wbg_init(module_or_path) {
    ...
    if (typeof module_or_path === 'undefined') {
        module_or_path = new URL('polysubml_demo_bg.wasm', import.meta.url);
    }
    const imports = __wbg_get_imports();
