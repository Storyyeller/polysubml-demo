...
pub enum TestStatus {
    Pass,
    CompilationMismatch { actual: CompilationResult, expected: CompilationResult },
    ExecutionMismatch { actual_output: String, expected_output: String },
    BaselineNotFound,
}
...
impl RegressionTester {
    ...
    fn compile_test(&self, test_path: &Path) -> Result<CompilationResult> {
        let test_content = fs::read_to_string(test_path)
            .map_err(|e| anyhow!("Failed to read test file {}: {}", test_path.display(), e))?;

        let mut state = State::new();
        let result = if state.process(&test_content) {
            CompilationResult::Success(state.get_output().unwrap())
        } else {
            CompilationResult::Error(
                state.get_err().unwrap_or_else(|| "Unknown compilation error".to_string())
            )
        };
    ...
    fn execute_js(&self, compiled_code: &str) -> Result<String> {
        ...
        if !output.status.success() {
            return Err(anyhow!("Node.js execution failed: {}",
    ...
    fn print_diff(&self, result: &TestResult) {
        ...
        match &result.status {
            ...
            TestStatus::ExecutionMismatch { actual_output, expected_output } => {
    ...
    fn print_summary(&self, results: &[TestResult]) {
        ...
        let skipped = results.iter().filter(|r| matches!(r.status, TestStatus::BaselineNotFound)).count();
        let failed = total - passed - skipped;

        println!("\nTest Results: {} passed, {} failed, {} skipped, {} total", passed, failed, skipped, total);

        if failed > 0 {
            println!("\nFailed tests:");
            for result in results.iter().filter(|r| matches!(r.status, TestStatus::CompilationMismatch { .. } | TestStatus::ExecutionMismatch { .. })) {
